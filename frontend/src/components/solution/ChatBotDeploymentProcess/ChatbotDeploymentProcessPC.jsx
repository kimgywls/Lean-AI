...
export default function ChatbotProcessPC() {
  const steps = [
    {
      title: "솔루션 상담",
  ...
  return (
    <div className="hidden md:flex flex-col space-y-4 md:space-y-8 rounded-xl bg-gray-50 p-5">
      {steps.map((step, index) => (
        <div key={index} className="relative">
          <div className="flex flex-col md:flex-row space-y-2 items-start">
            <div
              className={`p-2 md:p-3 bg-white rounded-full text-white flex-shrink-0 z-10`}
            >
              {step.icon}
            </div>
